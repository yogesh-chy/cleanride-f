...
import { bookingService } from "@/lib/booking-service";
 ...
 export default function FinancePage() {
   ...
   const loadStats = async () => {
     const data = await bookingService.fetchStats();
     setStats(data);
   };
   ...
   const handleExport = () => {
     ...
     exportToCSV(
       ...
       `cleanride-finances-${new Date().toISOString().split('T')[0]}.csv`
     );
     toast.success("Financial data exported successfully!");
   };
 
   // Map backend data to chart format
   const chartData = activeFilter === "Packages" 
     ? stats.package_breakdown 
     : stats.vehicle_breakdown;
 
   const statsCards = [
     { label: "Total Revenue", value: `Rs. ${stats.total_revenue.toLocaleString()}`, sub: "all time", icon: <DollarSign size={18} />, trend: "+100%", trendUp: true },
     { label: "Bookings", value: stats.total_bookings.toString(), sub: "total count", icon: <BarChart3 size={18} />, trend: "LIVE", trendUp: true },
     { label: "New Customers", value: stats.new_customers_today.toString(), sub: "joined today", icon: <ArrowUpRight size={18} />, trend: "NEW", trendUp: true },
     { label: "Active Jobs", value: stats.active_bookings.toString(), sub: "in progress", icon: <PieChart size={18} />, trend: "NOW", trendUp: true },
   ];
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
         {statsCards.map((stat, i) => (
           ...
           >
             ...
             <h2 className="font-heading text-xl text-foreground mb-1 tracking-tight">{stat.value}</h2>
       ...
       >
         <div className="flex items-center justify-between mb-8">
           <div>
             ...
             <p className="font-body text-xs text-muted-foreground mt-1">Revenue distribution by service package (all time)</p>
           </div>
         ...
         <div className="p-6 border-b border-border flex items-center justify-between">
           ...
           <div className="flex items-center gap-2">
             <div className="relative">
               ...
               <button onClick={() => loadStats()} className="pl-9 pr-4 py-2 rounded-md bg-secondary border border-border text-[10px] font-heading tracking-widest text-foreground uppercase">Refresh Data</button>
             </div>
           </div>
         </div>
           ...
           <table className="w-full">
             ...
             <tbody className="divide-y divide-border">
               ...
               {stats.recent_transactions.length === 0 && (
                 <tr>
                   <td colSpan={6} className="px-6 py-10 text-center text-muted-foreground font-body text-sm">No transactions found</td>
                 </tr>
               )}
             </tbody>
           </table>
